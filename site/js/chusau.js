...
app.filter('ASTRANGTHAI',function(){
 ...
 app.controller("Chusau",function($scope, $http,$compile,$filter) {
	 $scope.limit = 100;
	 $scope.offset =0;
...
$scope.TT3=0;
	$scope.FieldsRead = function(){
		   $http.post("index.php?option=com_katas&task=Chusau.FieldsRead&format=raw")  
	...
	$scope.FieldsCreate = function(){
		   $http.post("index.php?option=com_katas&task=Chusau.FieldsCreate&format=raw",{'dulieu':$scope.dulieu,'idUser':$scope.idUser})  
			.then(function(response) { 
	...
	$scope.FieldsUpdate = function(){
		   $http.post("index.php?option=com_katas&task=Chusau.FieldsUpdate&format=raw",{'dulieu':$scope.dulieu})  
			.then(function(response) { 
			   console.log(response);
			   $scope.FieldsRead();
			$scope.Reset();
	...
	$scope.FieldsRemove = function(dulieu){
		   $http.post("index.php?option=com_katas&task=Chusau.FieldsRemove&format=raw",{'dulieu':dulieu})  
			.then(function(response) { 
			  $scope.FieldsRead();
			  $scope.loai = response.data.loai; 
			  $scope.noidung = response.data.noidung; 
