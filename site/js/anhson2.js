...
$scope.TT3=0;
	$scope.FieldsRead = function(){
		   $http.post("index.php?option=com_katas&task=Anhson2.FieldsRead&format=raw")  
	...
	$scope.FieldsCreate = function(){
		   $http.post("index.php?option=com_katas&task=Anhson2.FieldsCreate&format=raw",{'dulieu':$scope.dulieu,'idUser':$scope.idUser})  
			.then(function(response) { 
	...
	$scope.FieldsUpdate = function(){
		   $http.post("index.php?option=com_katas&task=Anhson2.FieldsUpdate&format=raw",{'dulieu':$scope.dulieu})  
			.then(function(response) { 
			   console.log(response);
			   $scope.FieldsRead();
			$scope.Reset();
	...
	$scope.FieldsRemove = function(dulieu){
		   $http.post("index.php?option=com_katas&task=Anhson2.FieldsRemove&format=raw",{'dulieu':dulieu})  
			.then(function(response) { 
			  $scope.FieldsRead();
			  $scope.loai = response.data.loai; 
			  $scope.noidung = response.data.noidung; 
